...
def run_extract(site_key: str):
    ...
    if config.get("sitemap"):
        ...
        # Filter to only detail pages (not listing page itself)
        if site_key == "huurwoningen_rotterdam":
            ...
            # Exclude kaart (map) pages and keep only actual project pages
    ...
    # Site-specific URL filtering
    if site_key == "heijmans_nl":
        # Only include /projecten/{slug}/ URLs (actual projects, not index)
    ...
    merged_data = merge_data(new_records, existing_data, config.get("unique_key", "url"))
    metadata = {
        "site": site_key,
        "stage": "extract",
...
def run_merge():
    """Merge all sites' outputs into unified format."""
    logger = setup_central_logging(LOG_DIR)
...
if __name__ == "__main__":
    ...
    site_parser = subparsers.add_parser("site", help="Run pipeline for a specific site")
    site_parser.add_argument("--site", required=True, help="Site key (nieuwbouw_nl, houseatthepark_nl, heijmans_nl)")
    site_parser.add_argument("--stage", choices=STAGE_CHOICES, default="extract", help="Run only extract, transform, load or full pipeline")
    
    # Merge command
    merge_parser = subparsers.add_parser("merge", help="Merge all sites into unified format")
    
    # Full pipeline (legacy format support)
